...
const g_elementDivUserInfo_outer = document.getElementById("outer-video");
...
const is_client = document.getElementsByTagName('meta')[0].getAttribute("type")=="client"?true:false;
...
function onsubmitButton_Join(rm) {
  console.log("UI Event : 'Join' button clicked. >>" + rm);

  // ユーザー名
  // let strInputUserName = g_elementInputUserName.value;
  let strInputUserName = "u_"+rm;
  console.log("- User name :", strInputUserName);
  if (!strInputUserName) {
    return;
  }
...
function onclickButton_LeaveChat() {
  ...
  g_mapRtcPeerConnection.forEach((rtcPeerConnection) => {
    ...
    console.log("Call : endPeerConnection() >"+rtcPeerConnection.strRemoteSocketID);
...
g_socket.on("rooms_res", (room_list) => {
  ...
  for (let rm of r) {
    ...
    appendRoomInfoElement(rm);
  }
});

function appendRoomInfoElement(room) {
  let form = document.createElement("form");
  // form.method = 'POST';
  form.action = "";
  form.onsubmit = function (e) {
    onsubmitButton_Join(room[0]);
    return false;
  }
...
function appendRemoteInfoElement(strRemoteSocketID, strUserName) {
  ...
  elementVideo.setAttribute('class', 'dock-video');
  ...
  elementDiv.setAttribute('class', 'canvas');
  ...
  // outer.replaceWith(elementDiv);
  g_elementDivUserInfo_outer.appendChild(elementDiv);
}
...
function removeRemoteInfoElement(strRemoteSocketID) {
  ...
  // 要素の削除
  g_elementDivUserInfo_outer.removeChild(elementTable);
}
